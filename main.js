...
function chinfo(msg) {
  if(msg.content.substr(8) === '') {
    ...
    msg.channel.sendMessage('Info changed to: ```'+db.info[msg.author.id]+'```');
  }
  saveFile(file);
}
...
function slap(msg) {
  if(msg.content.substr(6) === "") {
    msg.channel.sendMessage('<@'+msg.author.id+'> slaps around a bit with a large trout');
  }
...
function fortune(msg) {
  fortunes.search(function(results){
    msg.channel.sendMessage('```'+results[1]+'```');
  });
}
function step(msg) {
  if(msg.content.substr(6) === "") {
    msg.channel.sendMessage('<@'+msg.author.id+'> steps');
  }
  else if(msg.content.substr(6,2) === "on" && !/everyone/g.test(msg.content) && !/here/g.test(msg.content)) {
    msg.channel.sendMessage('<@'+msg.author.id+'> steps on '+ msg.content.substr(9));
  }
  else if(!/everyone/g.test(msg.content) && !/here/g.test(msg.content)) {
...
function sudo(msg) {
  if(!msg.member.roles.exists('name', 'sudoers')) {
    ...
    msg.guild.channels.find('319978598339444736', 'var-spool-mail').sendMessage('<@'+msg.author.id+'> is on santa\'s naugthy list and will receive coal for Christmas.');
    return;
  }
  if(msg.content.substr(8, 4) === 'info') {
    ...
    if(msg.content.substr(6, 6) === 'chinfo') {
      ...
      msg.channel.sendMessage('Info changed to: ```'+db.info[userId]+'```');
    }
    ...
    msg.channel.sendMessage('<@'+msg.author.id+'> slaps '+ msg.content.substr(11) +' around a bit with a large trout');
  }
}
